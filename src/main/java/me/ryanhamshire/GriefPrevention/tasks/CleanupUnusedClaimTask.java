...
public class CleanupUnusedClaimTask implements Runnable {
    ...
    public void run() {
        ...
        if (GriefPrevention.plugin.config_claims_automaticClaimsForNewPlayersRadius >= 0) {
            areaOfDefaultClaim = (int) Math.pow(GriefPrevention.plugin.config_claims_automaticClaimsForNewPlayersRadius * 2 + 1, 2);
        }
        ...
        if (ownerData.getClaims(true).size() == 1 && claim.getArea() <= areaOfDefaultClaim && GriefPrevention.plugin.config_claims_chestClaimExpirationDays > 0) {
            ...
            if (sevenDaysAgo.getTime().after(new Date(ownerInfo.getLastPlayed()))) {
                ...
                GriefPrevention.plugin.dataStore.deleteClaim(claim, true, true);
        ...
        //if configured to always remove claims after some inactivity period without exceptions...
        else if (GriefPrevention.plugin.config_claims_expirationDays > 0) {
            Calendar earliestPermissibleLastLogin = Calendar.getInstance();
            earliestPermissibleLastLogin.add(Calendar.DATE, -GriefPrevention.plugin.config_claims_expirationDays);
        ...
        else if (GriefPrevention.plugin.config_claims_unusedClaimExpirationDays > 0 && GriefPrevention.plugin.creativeRulesApply(claim.getLesserBoundaryCorner())) {
            ...
            if (investmentScore < minInvestment) {
                //if the owner has been gone at least a week, and if he has ONLY the new player claim, it will be removed
                Calendar sevenDaysAgo = Calendar.getInstance();
                sevenDaysAgo.add(Calendar.DATE, -GriefPrevention.plugin.config_claims_unusedClaimExpirationDays);
