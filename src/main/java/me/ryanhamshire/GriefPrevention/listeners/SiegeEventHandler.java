    ...
    public void onClaimPermissionCheck(ClaimPermissionCheckEvent event) {
        ...
        if (event.getRequiredPermission() == ClaimPermission.Edit) {
            if (claim.siegeData != null)
                event.setDenialReason(() -> GriefPrevention.plugin.dataStore.getMessage(Messages.NoModifyDuringSiege));
            return;
        }
        ...
        if (event.getRequiredPermission() == ClaimPermission.Inventory) {
            ...
            GriefPrevention.plugin.dataStore.tryExtendSiege(player, claim);

            if (claim.siegeData != null)
                event.setDenialReason(() -> GriefPrevention.plugin.dataStore.getMessage(Messages.NoContainersSiege, claim.siegeData.attacker.getName()));
        ...
        if (broken != null) {
            ...
            if (!GriefPrevention.plugin.config_siege_blocks.contains(broken))
                event.setDenialReason(() -> GriefPrevention.plugin.dataStore.getMessage(Messages.NonSiegeMaterial));
            else if (player.getUniqueId().equals(claim.ownerID))
                event.setDenialReason(() -> GriefPrevention.plugin.dataStore.getMessage(Messages.NoOwnerBuildUnderSiege));
            return;
        }

        // No building while under siege.
        if (claim.siegeData != null)
            event.setDenialReason(() -> GriefPrevention.plugin.dataStore.getMessage(Messages.NoBuildUnderSiege, claim.siegeData.attacker.getName()));
