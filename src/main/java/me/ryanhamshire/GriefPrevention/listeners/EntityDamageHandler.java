...
public class EntityDamageHandler implements Listener {
    ...
    private boolean handlePvpPetDamageByPlayer(
        ...
        if (instance.config_pvp_protectPets) {
            ...
            PreventPvPEvent pvpEvent = new PreventPvPEvent(new Claim(null, DataStore.locationToClaimCorner(event.getEntity().getLocation()), DataStore.locationToClaimCorner(event.getEntity().getLocation()), null, new HashMap<>(), new HashMap<>(), new ArrayList<>(), 0, false, new HashMap<>(), null, new ArrayList<>()), attacker, pet);
